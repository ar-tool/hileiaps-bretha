...
uniform float u_reflectFactor;
varying vec3 reflectvec;
#endif

#if defined(CARPAINT)
uniform vec4 u_carPaintColor;
#endif
...
void main()
{
	...
	#if defined(LIGHTMAP)
	vec4 lightColor = texture2D(u_diffuseTexture, v_texCoord);
	gl_FragColor.rgb = gl_FragColor.rgb * lightColor.rgb;
	#endif
    ...
    #if defined(CUBEMAP)
    vec4 cubeMapColor = textureCube(u_cubemapTexture, reflectvec.stp);    
    gl_FragColor = mix(gl_FragColor, cubeMapColor, u_reflectFactor);
    #endif
